...
class Training():
    ...
    def backprop_chat(self, chat_grads, id, step):
    ...
    #     siz = 0.1
        for i in range(N_BP_MESSAGES):
            ...
            with self.tapes[msg_agent_id][msg_id]:
                msg = self.messages[msg_agent_id][msg_id] * chat_grads[:, :, :, indexes[i]] * 1e4 # arbitrary number to combat vanishing gradient by upscaling everything to roughly the same scale as the first model backprop
    ...
    #                 siz += np.array(m_g[i]).size
            ...
            self.add_grads(m_g, c_m_g, id, step)
    #     print(al/siz)
    ...
    #     tim = time.time()
        self.backprop_chat(chat_grads, id, step)
        f = time.time() - tim
        tim = time.time()
        ...
        self.tapes[id][0] = new_tape
        g = time.time() - tim
        print("all", round(a+b+c+d+e+f+g, 3), "get_vars", round(a, 3), "watch", round(b, 3), "forward", round(c, 3), "filter", round(d, 3), "losscomp+close", round(e, 3), "grads", round(f, 3), "addgrads", round(g, 3))
